...
static void audio_callback(
        daisysp::AudioHandle::InputBuffer in,
...
int main() {
    ...
    onset_detection.init(
        OnsetDetectionParams(
            num_bands=hw.AudioBlockSize();
        )
    );
    tempo_extraction.init(
        TempoExtractionParams(
            start_bpm=50,
            step_bpm=10,
            filter_q_factor=30,
    ...
    phase_extraction.init(
        PhaseExtractionParams(
            num_samples=onset_buffer.num_samples,
            sample_rate=decimation_rate
        )
    );
